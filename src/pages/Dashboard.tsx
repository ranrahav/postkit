...
const handleCreateCarousel = async () => {
    ...
    try {
      ...
      if (isPostComplete) {
        ...
        // Call OpenAI to generate the post content
        const { data: postData, error: postError } = await supabase.functions.invoke("generate-slides", {
          body: {
            text: newCarouselText,
            style: "Professional",
            language,
            content_type: "topic_idea",
            content_purpose: "thought_leadership",
          },
        });

        if (postError) {
          console.error("Error generating post content:", postError);
          throw postError;
        }

        // Use the AI-generated post content
        finalPostContent = postData.generated_post || postData.posts?.medium || newCarouselText;
        processedText = finalPostContent;
        
        console.log("🤖 Generated post content using OpenAI:", finalPostContent.substring(0, 100));
      } else {
